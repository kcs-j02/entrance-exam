...
/*
これはいきなりn*nにするとよくわからないため、5*5の問題に書いてあるやつで実際に書き出して試すのがやりやすい解き方
クイーンの効き筋が縦横斜め全ていける。そのため以下の方針で考えていく。
①まず同じ行と列には一つしか置けない
②斜めにも置けない
③おけるところがあればその時点で次の列に移動する→これによって全パターン調べるよりかなり無駄に調べる必要がなくなる。
④上記が当てはまらない箇所が効き筋上にないマスなのでreturn 1になる。（いわゆるそこにクイーンの駒を置くイメージ）
*/
int check(int x, int y) {
    int k;
    for (k = 0; k < x; k++) {
